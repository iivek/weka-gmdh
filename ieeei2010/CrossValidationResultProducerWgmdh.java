 ...
 */
public final class CrossValidationResultProducerWgmdh
        implements ResultProducer, OptionHandler, AdditionalMeasureProducer,
        RevisionHandler {

    private static final long serialVersionUID = -8093185701835261909L;
    ...
    protected RegressionSplitEvaluatorWgmdh m_SplitEvaluator =
            new RegressionSplitEvaluatorWgmdh();
    ...
    public boolean allDone = false; // signals if networks of all folds have
    // grown sufficiently
    private boolean[] done = new boolean[m_NumFolds];

    public void callMeBeforeStartingWithIterativeGrowing() {
        /*for (boolean b : done) {
         ...
         * wtf? this : sintax won't work
         */

        for (int cnt = 0; cnt < done.length; ++cnt) {
            done[cnt] = false;
        }
    }

    /**
     * Customized to allow for iterative growing of the network by one layer
     * and evaluating its performance at the same time. Loads serialized
     * Mscs and invokes next(), which builds an extra layer off the existing
     * network.
     * Otherwise, the classifier should have been built from scratch.
     *
    ...
    public void doRun(int run) throws Exception {
        ...
        m_SplitEvaluator.setCrossvalidationFolds(m_NumFolds);

        int allDoneCounter = 0; // gets increased by one if a fold finishes
        ...
        for (int fold = 0; fold < m_NumFolds; fold++) {
            ...
            if (done[fold]) {
                ++allDoneCounter;
                //System.out.println("    skipping " + allDoneCounter);
                if (allDoneCounter == m_NumFolds) {
                    allDone = true;
                    // reset done
                    return;
                }
            ...
            if (m_ResultListener.isResultRequired(this, key)) {
                ...
                try {
                    m_SplitEvaluator.setCrossvalidationFold(fold);

                    /* CHANGES BEGIN
                     */
                    // send the filename part to (RegressionSplitEvaluatorWgmdh) m_SplitEvalator,
                    // in which the performances of gmdh will be stored, layer by layer

                    /*((RegressionSplitEvaluatorWgmdh) m_SplitEvaluator).setFilenameMarker(
                            "" + m_Instances.relationName() + " " + run + " " + fold);
                     */
                    ((RegressionSplitEvaluatorWgmdh) m_SplitEvaluator).setFilenameMarker(
                            "" + m_Instances.relationName() + " " + filenameClassifierPart + " " + run + " " + fold);
                    // can key be used instead? or resultName from 2 blocks below_
                    ...
                    if (seResults == null) {                        
                        ...
                        //System.out.println("    Post minimum learning finished ");
                        ...
                        String resultName;
                        if (m_debugOutput) {
                            resultName = ("" + run + "." + (fold + 1) + "." + Utils.backQuoteChars(runInstances.relationName()) + "." + m_SplitEvaluator.toString()).replace(' ', '_');
    ...
    private String filenameClassifierPart;
    public void passFilenameClassifierPart(String s)  {
        filenameClassifierPart = s;
    }

    /**
     * Similar to overridden doRun. The difference is that it evaluates only one
     * MscNodepthParameter classifier per fold, setting their depth parameters
     * to values that correspond to minima on the control sets.
     * The difference from CrossValidationResultProducer.doRun is that it calls
     * m_SplitEvaluator.getResultDepthPreset instead of m_SplitEvaluator.getResult
     *
     * @param run the run number to get results for.
     * @throws Exception if a problem occurs while getting the results
     */
    public void doRunDepthPreset(int run) throws Exception {
        ...
        for (int fold = 0; fold < m_NumFolds; fold++) {
            ...
            //System.out.println("CVRP optimal depth preset, fold = " + fold);
            // Add in some fields to the key like run and fold number, dataset name
            Object[] seKey = m_SplitEvaluator.getKey();
            Object[] key = new Object[seKey.length + 3];
            key[0] = Utils.backQuoteChars(m_Instances.relationName());
            ...
            if (m_ResultListener.isResultRequired(this, key)) {
                ...
                try {
                    m_SplitEvaluator.setCrossvalidationFold(fold);
                    Object[] seResults = m_SplitEvaluator.getResultDepthPreset(train, test);
     ...
     */
    public RegressionSplitEvaluatorWgmdh getSplitEvaluator() {
